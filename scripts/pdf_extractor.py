...
def extract_pdf_text(file_path: str) -> Optional[str]:
    ...
    print(f"    Trying OCR for scanned PDF...")
...
def extract_pdfs_from_page(html_content: str, page_url: str, session: requests.Session) -> List[Dict]:
    ...
    for pdf_url in pdf_links:
        ...
        # Include PDF even if no text could be extracted (might be useful as reference)
        if text is None:
            text = f"[PDF konnte nicht gelesen werden - möglicherweise ein Bild-PDF]: {pdf_url}"

        if len(text.strip()) > 10:  # Very lenient - even if just a filename or URL
            ...
            print(f"    ✗ PDF too small or empty: {pdf_url}")
